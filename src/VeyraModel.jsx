...
function Face({ state }) {
  ...
  useFrame(({ clock }) => {
    const t = clock.elapsedTime
    const blink = Math.sin(t*.72) > .992 ? .12 : 1
    const cfg = STATE[state]
    const y = cfg.eye * blink
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (head.current) {
      ...
      head.current.rotation.z = THREE.MathUtils.lerp(
        ...
        target + Math.sin(t*.9)*.015,
        .04
      )
    }

    if (halo.current) {
      halo.current.rotation.z += delta * cfg.halo * haloControl
    }

    if (halo2.current) {
      halo2.current.rotation.z -= delta * cfg.halo * .68 * haloControl
    }

    if (orbitA.current) {
      orbitA.current.rotation.z += delta * cfg.orbit
    }

    if (orbitB.current) {
      orbitB.current.rotation.z -= delta * cfg.orbit * .65
    }

    if (lowerRing.current) {
      lowerRing.current.rotation.z += delta * (.8 + cfg.halo*.45)
    }
    ...
    <group ref={root} scale={1.12} position={[0,-.2,0]}>
      <group ref={head}>
        ...
        <group ref={halo} position={[0,.05,2.20]} rotation={[.10,0,0]}>
          <Ring radius={.70} tube={.035} color={VIOLET} />
        </group>

        <group ref={halo2} position={[0,.05,2.20]} rotation={[.10,0,.32]}>
          <Ring radius={.52} tube={.018} color={CYAN} opacity={.9} />
        </group>
      </group>
      ...
      <group position={[0,-.49,.38]} rotation={[Math.PI/2,0,0]}>
        <Ring radius={.25} tube={.038} color={VIOLET} />
        <Ring radius={.17} tube={.022} color={CYAN} />
      </group>

      <mesh ref={core} position={[0,-.56,.38]}>
        <sphereGeometry args={[.12,32,20]} />
        <meshStandardMaterial {...mat(CYAN,CYAN,3,.08,.08)} />
      </mesh>
      ...
      {[-1,1].map(side => (
        <group key={side} position={[.92*side,.18,.85]}>
          ...
          <mesh position={[0,-.10,0]} rotation={[Math.PI/2,0,0]}>
            ...
            <meshStandardMaterial {...mat(CYAN,CYAN,2.2,.1,.1)} />
          </mesh>
        </group>
      ))}
      ...
      <group ref={lowerRing} position={[0,0,-.57]}>
        <Ring radius={.45} tube={.043} color={VIOLET} />
        <Ring radius={.29} tube={.024} color={CYAN} rotation={[.12,0,.2]} />
      </group>

      <mesh position={[0,0,-.59]}>
        <sphereGeometry args={[.11,24,16]} />
        <meshStandardMaterial {...mat(CYAN,CYAN,2.8,.08,.08)} />
      </mesh>

      <group ref={orbitA} rotation={[.52,.22,.14]} position={[0,0,.42]}>
        <Ring radius={1.05} tube={.012} color={VIOLET} opacity={.72} />
      </group>

      <group ref={orbitB} rotation={[-.35,-.28,.12]} position={[0,0,.48]}>
        <Ring radius={.86} tube={.010} color={CYAN} opacity={.55} />
      </group>
